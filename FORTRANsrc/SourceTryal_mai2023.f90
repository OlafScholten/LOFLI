...
!     the link " X(  XIndx(i,i_Peak) ) = SourcePos(FitPos(i),i_Peak)" is not implemented.
 ...
 9 continue ! increase search distance for LMA antennas
   Write(2,*) i_eo,', cos_LMA(i_eo):', cos_LMA(i_eo),D_LMA(i_eo),', antennas:',LMAAnt(1:3,i_eo)
   ...
   Write(2,*) 'nr antennas=',i_sel,i_eo,', increase LMA-search distance to:',DistLMA
   flush(unit=2)
 8 continue
   Peak_nr=PeakNrTotal
   ...
   Do i_Peak=1,Peak_nr ! may be even or odd or both
      ...
      write(2,*) 'selected:', SelectAnt(1:i_sel)
      ...
      Write(2,*) i_Peak,i_eo,', cos_LMA(i_eo):', cos_LMA(i_eo),D_LMA(i_eo),', antennas:',LMAAnt(1:3,i_eo)
...
!=====================================
Subroutine LMA_Interfer(LMAAnt,Thet_LMA,Phi_LMA) ! (LMA_pos)
! called from 'SourceFind' with peak nr set to 1 or 2 as well as parities
!  Find an initial guess for the source position through a grid search.
!  Works only in imaging mode since covariance matrix is stored in this mode only. For this
!     the link " X(  XIndx(i,i_Peak) ) = SourcePos(FitPos(i),i_Peak)" is not implemented.
   ...
   use FitParams, only : SpaceCov !, Sigma, CalcHessian
   use Chunk_AntInfo, only : Ant_IDs, Ant_nr, Ant_pos, RefAnt
   use constants, only : dp,pi,c_mps, sample
   use DataConstants, only : Production
   ...
   integer :: i_chunk, i_peak, i_eo, i_ant, j_corr
   ...
   Do i_Peak=1,PeakNrTotal ! may be even or odd or both
      ...
      write(2,*) 'phi, cos_th:',phi(i_Peak), cos_th
      If(cos_th .gt.  0.99) cos_th=0.99
      If(cos_th .lt. -0.99) cos_th=0.99
...
!=====================================
Subroutine LMA_Interfer_obs(DistLMA,Thet_LMA,Phi_LMA) ! (LMA_pos)
! called from 'SourceFind' with peak nr set to 1 or 2 as well as parities
!  Find an initial guess for the source position through a grid search.
!  Works only in imaging mode since covariance matrix is stored in this mode only. For this
!     the link " X(  XIndx(i,i_Peak) ) = SourcePos(FitPos(i),i_Peak)" is not implemented.
   ...
   use DataConstants, only : Production
    use FFT, only : RFTransform_su,DAssignFFT
   Implicit none
   !Real(dp), intent(out) :: LMA_Pos(1:3,0:1)
   Real(dp), Intent(out) :: DistLMA,Thet_LMA,Phi_LMA
   integer :: i_chunk, i_peak, i_eo, i_ant, j_corr
   integer :: Peak_nr  !i,j,k,i_loc(1),
   Real(dp) :: RDist, D, D0=6.0d4, H0=1.0d4
   Real(dp) :: LMA_Pos(1:3,0:1)
 ...
 8 continue
   ...
   EndIf
   Return
End Subroutine LMA_Interfer_obs
!=====================================
Subroutine SourceTryal(DistMax,i_peakS)
!Subroutine SourceTryal_v2(DistMax)
!  Find an initial guess for the source position through a grid search.
!  Works only in imaging mode since covariance matrix is stored in this mode only. For this
!     the link " X(  XIndx(i,i_Peak) ) = SourcePos(FitPos(i),i_Peak)" is not implemented.
   !use Chunk_AntInfo, only : CTime_spectr, Ant_pos, Time_dim, Ant_Stations, Ant_IDs, Ant_nr
   ...
   Call LMA_Interfer(LMAAnt,Thet_LMA,Phi_LMA)  ! changes SourcePos
   Do i_Peak=1,PeakNrTotal
...
!=====================================
Subroutine SourceTryal_v1(DistMax,i_peakS)
...
Subroutine SearchWin(i_ref, i_ant, i_chunk, SrcPos, EEst)
   ...
   !If(.not. Production) write(2,*) i_ref,i_ant,'SearchWin:',sqrt(EEst),MaxTimeWin,Da,Dr,Er1
