!=================================
Subroutine MDD_Fitter(X, Time_width, Space_Spread)
...
!=================================================
!=================================================
Subroutine CompareDD( meqn, nvar, X, nf, R, uiparm, urparm, ufparm )
...
!
!  Intut:
!     -X(1:nvar)=SourceStructure(0:3,1:M)  ! time and position w.r.t. (CenT_ms,CenLoc), units=samples,m
...
!         USe DSYEVR from the LAPACK library
...
!
  ...
  Real(dp) :: DelChi(-N_fit:+N_fit,1:2*Nr_IntFerMx)
   ...
   Integer, parameter :: Nt_l=500, NSrc_max=20
   Real(dp) :: G_IR(-Nt_l:Nt_l,1:NSrc_max,1:Ant_nrMax)
   Real(dp) :: dCenT(1:NSrc_max), dCenloc(1:3,1:NSrc_max)
   Real(dp) :: dt,ddt, D, G
   Integer :: NSrc, i, m, j_IntFer, idt, it_G, it_l
   NSrc=nvar/4
   i=1
   Do m=1,Nsrc
      dCenT(m)=X(i)
   ...
   Do j_IntFer=1,Nr_IntFer   ! Loop over selected antennas
      ...
      Do i=1,3
         Do j=1,3
            B(i,j)=Vec_p(i,j_IntFer) * Vec_p(j,j_IntFer) + Vec_t(i,j_IntFer)* Vec_t(j,j_IntFer)
         EndDo
      EndDo
      Do m=1,Nsrc
         Do n=1,Nsrc
