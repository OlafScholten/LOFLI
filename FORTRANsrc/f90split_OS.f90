...
!
!    John Burkardt
!July 2022, Update _OS to
!  Allow proper ending for modules containing functioons or subroutines
!  - account for 'pure' functions and subroutines
  implicit none

  integer ( kind = 4 ) iarg
  integer ( kind = 4 ) iargc
  character ( len = 255 ) input_file
  integer ( kind = 4 ) numarg,i,j,n,k

  !call timestamp ( )

  !write ( *, '(a)' ) ' '
  !write ( *, '(a)' ) 'Starting F90SPLIT'
...
!
  ...
  iarg = 1
  !write ( *, '(a,I3)' ) 'iarg:', iarg
  call getarg ( iarg, input_file )
  !write ( *, '(a,a)' ) 'Splitting file:',input_file
  call file_ext ( input_file, i, j )
  !write ( *, '(a,a,2I5)' ) 'file:',input_file, i, j
  n=1
  Do k=1,j-1
   If(( input_file(k:k).eq.'/') .or. ( input_file(k:k).eq.'\')) n=k+1
  EndDo
  i=i-2
  if ( i .lt.n ) i=n+1
!  OPEN(UNIT=2,STATUS='unknown',ACTION='WRITE',FILE='f90split.out')

  write ( *, '(a,a)' ) 'Splitting file:',input_file(n:i)//'.f90'
  OPEN(UNIT=2,STATUS='unknown',ACTION='WRITE',FILE='f90split_'//input_file(n:i)//'.out')
  write(2,*) i,j,k,n,input_file
  ...
  write ( 2, '(a)' ) ' '
  write ( 2, '(a,i4)' ) '  Number of files handled = ', numarg
!
!  Terminate.
!
  write ( 2, '(a)' ) ' '
  write ( 2, '(a)' ) 'F90SPLIT:'
  write ( 2, '(a)' ) '  Normal end of execution.'
...
!
  ...
  if ( ios /= 0 ) then
    write ( 2, '(a)' ) ' '
    write ( 2, '(a)' ) 'F90SPLIT - Fatal error!'
    write ( 2, '(a)' ) &
...
!
    if ( no_name ) then
      ...
      call f90_line_is_begin ( line, output_file, InsideModule )
...
!
    ...
    if ( f90_line_is_end ( line, InsideModule ) ) then
...
!
  ...
  write ( 2, '(a)' ) ' '
  write ( 2, '(a)' ) 'F90SPLIT:'
  write ( 2, '(a)' ) '  Reached end of ' // trim ( input_file )
  write ( 2, '(a,i8)' ) '  Lines read:              ', line_num
  write ( 2, '(a,i8)' ) '  Longest line length:     ', line_length_max
  write ( 2, '(a,i8)' ) '  Longest line location:   ', line_length_loc
  write ( 2, '(a,i8)' ) '  Named modules created:   ', module_num
  write ( 2, '(a,i8)' ) '  Lines sent to NO_NAME:   ', no_name_line_num
  if ( 0 < no_name_line_num ) then
    write ( 2, '(a,i8)' ) '  Last NO_NAME line:       ', no_name_line_last
  end if
  write ( 2, '(a,i8)' ) '  Duplicate modules found: ', duplicate_num
...
subroutine f90_line_is_begin ( line, name, InsideModule )
...
!
  ...
  call s_blank_delete ( line2 )
   write(2,*) line2(1:85)
  If( s_eqi(line2(1:4),'pure')) then  ! clip 'pure'
   line2(1:251)=line2(5:255)
   write(2,*) '*****',line2(1:30)
  EndIf
  If( s_eqi(line2(1:5),'real(')) then  ! clip 'real( .... )'
      n=0
      Do i=6,50
         If(line2(i:i).eq.')' ) then
            n=i+1
            line2(1:256-n)=line2(n:255)
            exit
         EndIf
      Enddo
      If(n.eq.0) stop 'closing brackets not found in f90split'
      write(2,*) '*****',line2(1:30)
  EndIf
...
function f90_line_is_end ( line, InsideModule )
...
!
  ...
  if ( &
    ...
    line2(1:4)  == 'end!') .and. .not.InsideModule) then

!write(*,*) 'Aline2:', line2(1:13),'f90_line_is_end:',f90_line_is_end,' InsideModule:',InsideModule
    f90_line_is_end = .true.
  ElseIf(    line2(1:9)  == 'endmodule' .and. InsideModule) then
!write(*,*) 'Bline2:', line2(1:13),'f90_line_is_end:',f90_line_is_end,' InsideModule:',InsideModule
    f90_line_is_end = .true.
    InsideModule = .false.
  EndIf
...
!
  ...
  write ( 2, '(i2,1x,a,1x,i4,2x,i2,a1,i2.2,a1,i2.2,a1,i3.3,1x,a)' ) &
