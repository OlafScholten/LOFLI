...
CONTAINS
!
Subroutine ListGrDaStructure
! List Group and Data structure of HDF% file,
! based on "h5ex_g_traverse_F03.f90" as distributed by the HDF group
  IMPLICIT NONE
  ...
  Return
END Subroutine ListGrDaStructure
...
!    USE ISO_C_BINDING
    ...
    !
    spaces = 2*(od%recurs+1) ! Gives the level of the group or data
    ...
    IF(infobuf%type.EQ.H5O_TYPE_GROUP_F)THEN
       ...
       ENDIF
       !
       WRITE(*,'(A)') space(1:spaces)//"}" ! Closing of a group
       RETURN
