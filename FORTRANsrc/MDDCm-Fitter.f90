...
1  Continue
   ...
   Do I_s=-Nt_l,Nt_l
      Do j_IntFer=1,Nr_IntFer !=Nr_IntFer   ! Loop over selected antennas
         ...
         EndDo !  m=1,Nsrc
         CE_MDD_p(j_IntFer)=E_p
         CE_MDD_t(j_IntFer)=E_t
         j= Nt_l+I_s+ (j_IntFer-1)*(2*T_Range)
         R(2*j+1)           =SQWght(j_IntFer)*RealPart( (CTime_p(MDDnuDim+I_s,j_IntFer) - E_p) )
         R(2*(j+ T_Range)+1)=SQWght(j_IntFer)*RealPart( (CTime_t(MDDnuDim+I_s,j_IntFer) - E_t) )
         R(2*j+2)           =SQWght(j_IntFer)*ImagPart( (CTime_p(MDDnuDim+I_s,j_IntFer) - E_p) )
         R(2*(j+ T_Range)+2)=SQWght(j_IntFer)*ImagPart( (CTime_t(MDDnuDim+I_s,j_IntFer) - E_t) )
...
!               DDChiSQ(1,1), DDChiSQ(1,2), Power_p, Power_t, Ch1, dCenloc(:,1) )
   ...
   EndIf
   !
   !Call GLEplotControl( Submit=.true.)  ;    stop 'CompareMDD'
   Return
