...
!
  ...
  if ( m <= 0 ) then
   ...
   !            write(2,*) 'IV(covmat) set to -1@842:',iv(ierr)
      iv(covmat) = -1
...
!  Set G = ( G - GSAVE ) / DEL.
!
  !             write(2,*) 'derivative:',v(g1:gp),m
  !             write(2,*) del,v(gsave1:gsave1+gp-g1)
      do i = g1, gp
...
!
      if ( m /= 1 ) then
  !             write(2,*) 'Hessian:',v(k:l),m
  !             write(2,*) 'added G:',v(g1:g1+l-k+p-m+1)
        do i = k, l
...
!
 ...
 !        write(2,*) '@964:',m,x(m),del,v(delta0)
    v(xmsave) = x(m)
    x(m) = x(m) + del
    v(delta) = del
    covirc = 2
...
!
  ...
  !    write(2,*) 'finite diff derivative:',m,x(m),del,v(dltfdc)
      v(xmsave) = x(m)
      x(m) = x(m) + del
      stpm = stp0 + m
      v(stpm) = del
...
!
    ...
    write(pu,*) "Hessian:"
    Do i=1,p
        write(pu,*) v(cov+i*(i-1)/2:cov+i*(i+1)/2-1)
    enddo
    endif
  !       Do i=1,p
  !             write(2,*) 'v(cov):',i,v(cov+i*(i-1)/2:cov+i*(i+1)/2-1)
  !       Enddo
!
...
!
  ...
  iv(covmat) = -1
 !              write(2,*) 'IV(covmat) set to -1@1152:',irc
  if ( irc /= 0 ) then
    return
  end if
...
!
  ...
  return
      ...
      SUBROUTINE GQTSTP(D, DIG, DIHDI, KA, L, P, STEP, V, W, Error)
...
!  SCALAR ARGUMENTS
      INTEGER &
         KA,P,Error
! Error = 1 when lost in endless loop at label=210
!
!  ARRAY ARGUMENTS
      DOUBLE PRECISION &
...
!
    ...
    !   print Hessian matrix
    !
    !write(2,"('Approximate Normalized Hessian ={D**-1 * H * D**-1}(KA=',i3,'):')") KA
    !Do i=1,p
    !    write(2,*) DIHDI(1+i*(i-1)/2:i*(i+1)/2)  ! same as Hessian printed later
    !enddo
!
...
!
 210  KA = KA + 1
      IF (KA-KALIM .GT. 100) THEN
        Write(*,*) 'MANY FAILURES IN GTQSTP, called in NL2SOL'
        Error=1
        return
        !STOP 'gqtstp of NL2SOL'
      END IF
...
!
!    if (pu /= 0) write(pu,"('Approximate Hessian not printed, KA=',i4,':')") KA
         RESTRT = .TRUE.
         KA = KA + 1
         K = 0
         DO 320 I = 1, P
...
subroutine nl2sol ( n, p, x, calcr, calcj, iv, v, uiparm, urparm, ufparm, error )
...
!      write(*,*) 'calcj'
 ...
 40   continue
     ...
     !write(2,*) 'nl2sol-itr1', x(1:10)
      call nl2itr ( v(d1), iv, v(j1), n, n, p, v(r1), v, x, error )
     !write(2,*) 'nl2sol-itr2', x(1:10)
     If(X(1).ne.X(1)) error=10   ! check for NaN
      If(error.gt.0) return

  if ( iv(1) == 2 ) then
!    write(2,*) 'gradient evaluation number', iv(30)  ! = iv(ngcall)
    go to 30
  end if
...
subroutine nl2itr ( d, iv, j, n, nn, p, r, v, x, error )
...
!
 ...
 160  continue
      ...
      !
      ! test writing
      !write(2,*) 'From nl2itr: iteration #',iv(niter)
      !write(*,*) 'From nl2itr: iteration #',iv(niter)
      !write(2,*) 'Approximate Hessian(1:p,1:p) = D^-1 (J^T J) D^-1, where J(1:n,1:p)=first derivative; D^d(1:p)=change in coeff'
      !write(2,*) 'D(1:p)=', (d(i), i=1,p)
      !do i = 1, p
      !  t = 1.0D+00 / d(i)
      !!  do k = 1, i
      !  write(2,*) 'row (i=',i,',1..i)= ',((t * dotprd(n,j(1,i),j(1,k)) / d(k)), k=1,i)
      !!  end do
      !end do
      ! end test writing
      !
...
subroutine timestampxxxx ( )  ! Obsolete? seems not to be used anywhere!
