...
!
   ...
   Complex(dp) :: PhIn_0p(m_dim,20), PhIn_0t(m_dim,20)
   Complex(dp) :: ThIn_p(1:j_dim,1:m_dim), ThIn_t(1:j_dim,1:m_dim)
   Complex(dp) :: ThIn_0p(1:j_dim,1:m_dim), ThIn_0t(1:j_dim,1:m_dim)
   !Real(dp) :: th, x, dx, dthet, S1p25, c1p25
   Complex(dp) :: Vp, Vt
   Integer :: nxx, it, i_freq, j
   Character(len=20) :: release
   INTEGER :: DATE_T(8),i
   CHARACTER*12 :: REAL_C(3)
   !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   OPEN(UNIT=2,STATUS='unknown',ACTION='WRITE',FILE='AntFunc-v3.out')
   ...
   Do while(nxx.eq.0)
      ...
      Call AngleIntegrate(thet,phi,vp,vt,PhIn_p,PhIn_t,ThIn_p,ThIn_t)
      If((Phi .gt. 359.) .and. (thet.gt.89.)) then
         Ant_p(i_freq,:,:)= ThIn_p(:,:) ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
         Ant_t(i_freq,:,:)= ThIn_t(:,:) ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
         Write(2,*) 'freq=',Freq
      Endif
   1  continue
   ...
   Do while(nxx.eq.0)
      i_freq=nint(Freq)
      if(i_freq .lt. it) goto 2
      if(i_freq .gt. it) exit
      ...
      Call AngleIntegrate(thet,phi,vp,vt,PhIn_p,PhIn_t,ThIn_p,ThIn_t)
      Call AngleIntegrate(thet,phi,J_0p(i_freq),J_0t(i_freq),PhIn_0p,PhIn_0t,ThIn_0p,ThIn_0t)
!      If(Phi .gt. 359.) then ! integrate over \phi since \phi=0 is same as 360
         !Write(2,"(F5.1,F7.1,4(2F10.4,2x))") Freq, Thet,  PhIn_p(:,it), PhIn_t(:,it)
         !Write(2,"(5x,A,4(2F10.4,2x))") 'model:', PhIn_0p(:,it), PhIn_0t(:,it)
!      Endif
      !
      If(phi.lt.1.)  Call TestAntFie(Freq,Thet,Phi,Vt)
      If(Nint(phi).eq.90)  Call TestAntFie(Freq,Thet,Phi,Vp)
      If(Phi.gt.90) goto 2
      if(nint(Phi).eq.0) then
         write(2,"(A,F6.1,2F9.4,2x,2F9.4)", ADVANCE='NO') 'thet=',thet,Vt,J_0t(i_Freq)
      endif
      if(nint(Phi).eq.90) then
         write(2,"(A,F6.1,2F9.4,2x,2F9.4)") 'thet=',thet,Vp,J_0p(i_Freq)
      endif
      !Call AntFun(thet,phi-45,J_0p,J_0t,J_1p,J_1t) ! Jones; gives _0 & _1 voltages when contracted with (t,p) polarized field
...
!============================
Subroutine AngleIntegrate(thet,phi,vp,vt,PhIn_p,PhIn_t,ThIn_p,ThIn_t)
   ! thet,phi in degree; thet in steps of 5 deg; phi in steps of 10 deg
   use constants, only : dp,pi,ci
   use AntFunCconst
   Implicit none
   Real(dp), intent(in) :: Thet, Phi
   Complex(dp), intent(in) :: Vp, Vt
   Complex(dp), intent(out) :: PhIn_p(m_dim,20), PhIn_t(m_dim,20)
   Complex(dp), intent(out) :: ThIn_p(1:j_dim,1:m_dim), ThIn_t(1:j_dim,1:m_dim)
   Real(dp) :: Cheb(m_dim), Cheb_s(m_dim), Lgdr(j_dim)
   !Real(dp) :: Freq0, Thet0, Phi0, Vp_r, Vp_i
   !Real(dp) :: Freq, Thet, Phi, Vt_r, Vt_i
   Real(dp) :: th, x, dx
   Real(dp), parameter :: S1p25=sin(1.25*pi/180.), c1p25=cos(1.25*pi/180.) ! needed for average angle first and last half bin
   Integer :: nxx, it, j
   Real(dp), parameter :: dthet=5.
   Integer, parameter :: nthet=nint(90./dthet)
   !S1p25=sin(1.25*pi/180.) ! needed for average angle first and last half bin
   !c1p25=cos(1.25*pi/180.)
   !
   it=nint(1. + thet/5.)
   If((Phi.lt.1.) .and. (thet.lt.1))  then
      PhIn_p(:,:)=0.
      PhIn_t(:,:)=0.
   Endif
   ...
   If(Phi .lt. 359.) then ! integrate over \phi since \phi=0 is same as 360
      ...
      !Write(2,"(F7.1,4F10.4,4F10.4)") Thet,  PhIn_p(:,it), PhIn_t(:,it)
      If(Thet.gt.89) then ! do \theta integral
         dx=(0.5*S1p25*pi*dthet/180.)*2/3.  ! get the average for the first bin (1/2 size)
         Call Legendre_odd(C1p25,Lgdr) ! x=cos(theta)
         Do j=1,j_dim
            ThIn_p(j,1:m_dim)= PhIn_p(1:m_dim,1)*Lgdr(j)*(4.*j-1.)*dx ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
            ThIn_t(j,1:m_dim)= PhIn_t(1:m_dim,1)*Lgdr(j)*(4.*j-1.)*dx ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
         Enddo
         ...
         ! because of the 5 deg spacing the accuracy of this integral is about 1% of the largest number
            ...
            Do j=1,j_dim
               ThIn_p(j,1:m_dim)=ThIn_p(j,1:m_dim) + PhIn_p(1:m_dim,it+1)*Lgdr(j)*(4.*j-1.)*dx ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
               ThIn_t(j,1:m_dim)=ThIn_t(j,1:m_dim) + PhIn_t(1:m_dim,it+1)*Lgdr(j)*(4.*j-1.)*dx ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
            Enddo
         Enddo
         write(2,"(A,4(2F8.4,2x))") ' Phi(re,im):=',ThIn_p(:,1)
         write(2,"(A,4(2F8.4,2x))") ' Thet(re,im):=',ThIn_t(:,1)
         !stop
      Endif
   Endif
   !
   Return
End Subroutine AngleIntegrate
!=======================
Subroutine TestAntFie(Freq,thet,Phi_d,V)
   use constants, only : dp,pi,ci
   use AntFunCconst
   Implicit none
   Real(dp), intent(in) :: Freq, Thet, Phi_d
   Complex(dp), intent(in) :: V
   Complex(dp) :: J0p, J0t
   Integer :: j,m,i_freq, unt
   ...
   Else
      unt=21
   Endif
   Write(unt,"(F5.3,F5.0,2(2F8.3,1x))", ADVANCE='NO') x,Thet,V
   Do j=1,j_dim ! corresponding to 1,3,5,7, Legendre polynominals
      Do m=1,m_dim ! corresponding to 1,3,5 Chebyshev polynominals
         J0p=J0p + Ant_p(i_freq,j,m)*Lgdr(j)*Cheb(m)
         J0t=J0t + Ant_t(i_freq,j,m)*Lgdr(j)*Cheb_s(m)
      Enddo ! m=1,2
      If(phi_d.gt.45) then
         write(unt,"(1x,2(2F8.3,1x))", ADVANCE='NO') J0p
      Else
         write(unt,"(1x,2(2F8.3,1x))", ADVANCE='NO') J0t
      Endif
   Enddo ! j=1,4
   write(unt,*) ' '
   !
   Return
End Subroutine TestAntFie
!========================
Subroutine TestLegendr(nthet)
   ...
   Integer :: j,it,nthet
   real(dp) :: dthet, Lgdr(j_dim)
   Real(dp) :: x,dx,S1p25,C1p25,th
   Real(dp) :: PhIn(1:m_dim),ThIn(1:j_dim,1:m_dim)
   ! Phi_r in radian, where Phi_r=0=along Y-dipole
   dthet=90./nthet
   S1p25=sin(dthet*pi/(180.*4))
   C1p25=cos(dthet*pi/(180.*4))
   dx=(0.5*S1p25*pi*dthet/180.  )*2/3. ! get the average for the first bin (1/2 size)
   Call Legendre_odd(C1p25,Lgdr) ! x=cos(theta)
   PhIn(1:m_dim)=Lgdr(1:m_dim)
   Do j=1,j_dim
      ThIn(j,1:m_dim)= PhIn(1:m_dim)*Lgdr(j)*(4.*j-1.)*dx ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
   Enddo
   Do it=1,nthet ! should integrate over d[x]=d[cos(th)]=sin(th) d[th]
      If(it.lt.nthet) then
         th=it*dthet*pi/180.
      ...
      PhIn(1:m_dim)=Lgdr(1:m_dim)
      Do j=1,j_dim
         ThIn(j,1:m_dim)=ThIn(j,1:m_dim) + PhIn(1:m_dim)*Lgdr(j)*(4.*j-1.)*dx ! norm: (2n+1)/18 where n=2j-1; 0.5 since interval at th=0 is only half width
      Enddo
   Enddo
   write(2,*) nthet,ThIn(:,1),ThIn(:,2)
   !
   Return
   !
End Subroutine TestLegendr
